...
static HttpServer::Connection::Target ParseTarget(const std::string& target) {
...
static HttpServer::Connection::HistoryFields GetHistoryFields(const std::string& query) {
    boost::smatch matches;
    if (!boost::regex_search(query, matches, boost::regex(R"(fields=["']?([\w|]+)["']?)"))) {
        return {};
    }

    HttpServer::Connection::HistoryFields fields = { false, false, false, false };
...
void HttpServer::Connection::getDisplay(int indentation) {
    json displayObject;
    displayObject["enabled"] = static_cast<bool>(*m_displayUi);
...
void HttpServer::Connection::getMaster(int indentation) {
    json masterObject;
    masterObject["enabled"] = static_cast<bool>(*m_captureMaster);
...
void HttpServer::Connection::postMaster(int indentation) {
    try {
        json requestJson = json::parse(beast::buffers_to_string(m_request.body().data()));
        bool enabled = requestJson.at("enabled");
        enabled ? m_captureMaster->enable() : m_captureMaster->disable();

        json masterObject;
        masterObject["enabled"] = enabled;
        ...
        m_response.set(beast::http::field::content_type, "application/json");
        beast::ostream(m_response.body()) << responseJson.dump(indentation) << '\n';
        m_logger->info(m_logMessage(fmt::format("OK: Capture master is {}", enabled ? "enabled" : "disabled")));
    }
    catch (const json::exception&) {
        json responseJson;
        responseJson["_success"] = false;
